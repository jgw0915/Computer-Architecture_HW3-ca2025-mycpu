    ...
    switch(io.funct3) {
      ...
      is(InstructionsTypeS.sh) {
        ...
        when(mem_address_index(1) === 0.U) {
          ...
          writeData := data(2 * Parameters.ByteBits - 1, 0) << (2 * Parameters.ByteBits)
        }
      }
